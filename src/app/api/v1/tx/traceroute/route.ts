...
import { enqueueTx, tracerouteRecentlyQueued } from "../../../../../db/tx.ts";
import { parseNodeId } from "../../../../../meshtastic/types.ts";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

// POST { to } queues a TRACEROUTE_APP request. Per-target cooldown keeps us a good citizen.
export async function POST(req: NextRequest) {
  ...
  if (await tracerouteRecentlyQueued(to, cfg.tx.traceroute_cooldown_s)) {
    return NextResponse.json({ error: "traceroute cooldown for this node" }, { status: 429 });
  }
  ...
  const id = await enqueueTx({
    createdBy: gate.actor, transport: cfg.tx.transport, kind: "traceroute", channelId: channel, toNode: to,
