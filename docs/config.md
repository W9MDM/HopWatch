...
| `ingest.decode.channel_keys[]` | `[{default, AQ==}, {LongTurbo, AQ==}]` | `{name, key}` base64 channel keys, including the default `AQ==` and LongTurbo (the US firmware default as of Meshtastic v2.8, same default key). The `name` is part of the wire identity, not a label: a key is tried only when `xorHash(name) ^ xorHash(key)` equals the 8-bit hash in `MeshPacket.channel`, so a key entered under the wrong channel name will not decrypt that channel. An empty `key` declares an unencrypted (PSK-less or ham-mode) channel, whose payload is plaintext and is read without decryption. |
